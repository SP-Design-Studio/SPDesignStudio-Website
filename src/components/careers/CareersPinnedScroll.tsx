...
export default function CareersPinnedScroll({ started }: { started: boolean }) {
	...
	const openingsWrap = useRef<HTMLDivElement>(null);
	const inviteWrap = useRef<HTMLDivElement>(null);
	...
	useEffect(() => {
		...
		const ctx = gsap.context(() => {
			...
			const reveals = (el: HTMLDivElement | null) =>
				el ? gsap.utils.toArray<HTMLElement>(el.querySelectorAll(".cr-reveal")) : [];
			const openingsEls = reveals(openingsWrap.current);
			const inviteEls = reveals(inviteWrap.current);

			gsap.set([openingsWrap.current, inviteWrap.current], { autoAlpha: 0 });
			...
			gsap.set([...openingsEls, ...inviteEls], {
				y: 28,
				autoAlpha: 0,
				filter: "blur(5px)",
			});
			...
			tl.to(openingsWrap.current, { autoAlpha: 1, duration: 0.01 }, 0.35)
				.to(
					openingsEls,
					{
						y: 0,
				...
				.to(openingsWrap.current, { autoAlpha: 0, duration: 0.01 }, 3.25)

				.to(inviteWrap.current, { autoAlpha: 1, duration: 0.01 }, 3.2)
				.to(
					inviteEls,
					{
						y: 0,
