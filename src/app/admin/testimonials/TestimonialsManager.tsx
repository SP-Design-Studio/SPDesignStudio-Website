	...
	return (
		<div data-busy={pending || undefined} className="grid grid-cols-1 sm:grid-cols-[140px_1fr] gap-4 rounded-sm border border-cream/10 bg-plum/20 p-4 transition-colors hover:border-cream/25">
			...
			<div className="flex flex-col gap-3">
				<label>
					<div className={ui.label}>Quote</div>
					<textarea
						rows={3}
				...
				<div className="grid grid-cols-2 gap-3">
					...
					<label>
						<div className={ui.label}>Detail</div>
						<input
							className={ui.input}
							value={form.detail}
				...
				<div className="mt-auto flex items-center justify-between">
					<div className="flex items-center gap-1">
						{reorderable && <DragHandle label="testimonial" />}
						<button
							aria-label="Move up"
							type="button"
...
export function TestimonialsManager({ initial }: { initial: Testimonial[] }) {
	...
	const search = useSearch(list, (t) => [t.name, t.quote, t.detail]);
		...
		<div className="flex flex-col gap-5">
			...
			{list.length === 0 && (
				<EmptyState
					...
					body="Client Reflections on the home page. Add quotes with the client's name and a portrait."
				/>
			)}
			{search.active && search.shown.length === 0 && (
				<p className="rounded-sm border border-dashed border-cream/15 px-4 py-8 text-center font-sans font-light text-cream/80 text-tiny">
					No testimonials match &ldquo;{search.q.trim()}&rdquo;.
				</p>
			)}
